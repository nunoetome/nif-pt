...
> Consulta de NIFs portugueses via [nif.pt](http://www.nif.pt) — validação Mod-11, pipeline Unix e persistência SQLite WAL + Azure SQL com camada resiliente de rate-limit.
...
```
consulta_nif.py  ──JSON──>  importar_nif_sqlite.py  (SQLite local, JSON bruto, WAL)
                 ──JSON──>  importar_nif.py         (Azure SQL, 36 cols normalizadas)
                 ──erro──>  nif_api_erros           (SQLite + Azure, auditoria + retry)
```

* **Validação** Mod-11 da AT (`consulta_nif.py:48` `validar_nif`) antes de consumir créditos.
* **Resiliência:** `utils/error_handler.py` (612L) classifica `rate_limit_minute/hour/day/month/paid` → `retry 60s/3600s` vs `abort`, persiste sempre em `nif_api_erros` (dual SQLite `PRAGMA WAL` + Azure `sql/02_criar_tabela_erros.sql`), limites `max_tentativas_global 5 / minuto 3 / hora 2 / dia 0 / mes 0` em `config.yaml`.
* **SQLite** `data/nif_pt.db` WAL — `nif_pt` (4 cols) + `nif_api_erros` (14 cols) — schemaless, à prova de mudanças da API.
* **Azure SQL** `kiwa-pt-operations/stg_nunotome` — `nif_pt` 37 cols + `nif_pt_stg` 40 cols + `nif_api_erros` 14 cols — analítico, `pyodbc` + `ODBC Driver 18`.
...
Ver [MANUAL.md](MANUAL.md) (v1.0.0) para manual completo e [docs/technical.md](docs/technical.md) para arquitetura técnica com Mermaid.
...
```mermaid
graph LR
    U[Utilizador<br/>NIF 9 dígitos] --> C[consulta_nif.py<br/>validar Mod-11 + requests<br/>retry per-tipo+global]
    ...
    C -->|tipo_erro| E[(nif_api_erros<br/>SQLite + Azure)]
    S --> DB1[(data/nif_pt.db<br/>nif_pt + nif_api_erros)]
    A --> DB2[(Azure SQL<br/>stg_nunotome<br/>nif_pt 37c + nif_pt_stg 40c<br/>+ nif_api_erros 14c)]
...
```
nif-pt/
├── consulta_nif.py               # 355L — validar Mod-11 + GET ?json=1 + retry per-tipo+global + BOX
├── importar_nif.py               # 321L — mapear 36 cols → Azure SQL staging (pyodbc)
├── importar_nif_sqlite.py        # 152L — JSON bruto → SQLite WAL (4 cols)
├── utils/error_handler.py        # 612L — classificar_erro + tratar_erro/handle_error + guardar_erro + init_error_table
├── Logging/logging_orchestrator.py # 580L — RotatingFileHandler 10MB/5000/10, R1-R9, <<nif-pt>>, nif_pt.log
├── config/
...
├── sql/
│   ├── 01_criar_tabelas.sql      # 157L — nif_pt 37c + nif_pt_stg 40c
│   └── 02_criar_tabela_erros.sql # 44L — nif_api_erros 14c dual SQLite/Azure
├── docs/
│   ├── technical.md              # v1.0.0 — C4 + sequência + ER + catálogo file:line + ADRs
│   ├── api/help.md               # Help Python verificado (help(validar_nif), help(tratar_erro))
│   ├── architecture/             # C4 + diagramas Mermaid
│   ├── database/                 # ER + catálogo 37/40/14 cols
│   ├── api/nif-pt-api.md         # Contrato nif.pt
│   └── guides/                   # instalação, logging R1-R9
...
```powershell
# Só consultar (JSON stdout, stderr com BOX/TAG/TIMING)
python consulta_nif.py 509442013 > resultado.json
Get-Content resultado.json | ConvertFrom-Json | Format-List
...
Ver `docs/guides/logging.md` e `docs/technical.md` §11.

## Documentação

| Documento | Descrição |
|-----------|-----------|
| [MANUAL.md](MANUAL.md) | Manual de utilizador v1.0.0 (15 capítulos, instalação Windows, pipe, troubleshooting, FAQ) |
| [docs/technical.md](docs/technical.md) | Documentação técnica v1.0.0 — C4, sequência, ER, catálogo file:line, ADRs |
| [docs/api/help.md](docs/api/help.md) | Help Python (PEP 257) — `help(validar_nif)`, `help(tratar_erro)` verificado |
| [docs/architecture/architecture.md](docs/architecture/architecture.md) | Arquitetura C4 + 8 ADRs |
| [docs/database/schema.md](docs/database/schema.md) | ER + catálogo 37/40/14 cols |
...
`v1.0.0` (2026-09-10) — primeiro stable: camada `error_handler` + `nif_api_erros` dual + `logging_orchestrator` integrado + `timeout 10s` + `max_tentativas_*` + docs v1.0.0. Ver [CHANGELOG.md](CHANGELOG.md).
